 ...
 public:
  ...
  /// Header for hashtable entries
  {
   public:
    EntryHeader* next;
    hash_t hash;
    EntryHeader(EntryHeader* n, hash_t h)
        : next(n),
          hash(h) {
    }
  ...
  inline void PrefetchEntry(size_t hash) {
    _mm_prefetch(((char* )(entries + (hash&mask))), _MM_HINT_T0);
  }
...
inline Hashmap::EntryHeader* Hashmap::end() {
  return nullptr;
}
...
inline Hashmap::EntryHeader* Hashmap::update(Hashmap::EntryHeader* old, Hashmap::EntryHeader* p, Hashmap::hash_t hash) {
...
class Hashmapx : public Hashmap {
 ...
 // static const uint64_t seed = 902850234;
  ...
  V* findOrCreate(KEY&& key, hash_t hash, V& defaultValue, T& entryCollection, CB onGroup);
...
void Hashmapx<K, V, H, useTags>::insertAll(std::deque<Entry>& entries) {
  if (useTags)
    for (auto& e : entries)
      insert_tagged<concurrentInsert>(&e.h, e.h.hash);
  else
    for (auto& e : entries)
...
template<typename K, typename V, typename H, bool useTags>
template<typename T>
inline V* Hashmapx<K, V, H, useTags>::findOrCreate(K& key, hash_t hash, V& defaultValue, T& entryCollection) {
...
template<typename T, typename CB, typename KEY>
inline V*
Hashmapx<K, V, H, useTags>::findOrCreate(KEY&& key, hash_t hash, V& defaultValue, T& entryCollection, CB onGroup) {
...
Hashset<K, H, useTags>::Entry::Entry(hash_t h, K key)
    : h { nullptr, h },
      k(key) {
}
