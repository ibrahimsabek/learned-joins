/* An adapted implementation of SSB query q32 based on https://github.com/fzhedu/db-imv */
...
NOVECTORIZE std::unique_ptr<runtime::Query> q32_hyper(Database& db,
                                                      size_t nrThreads) {
   // --- aggregates
...
std::unique_ptr<Q32Builder::Q32> Q32Builder::getQuery() {
   using namespace vectorwise;
   auto result = Result();
   previous = result.resultWriter.shared.result->participate();
   auto r = make_unique<Q32>();
   ...
   Select(
       Expression().addOp(BF(primitives::sel_equal_to_Char_15_col_Char_15_val),
                          ...
                          Column(customer, "c_nation"), Value(&r->nation)));
...
std::unique_ptr<runtime::Query> q32_vectorwise(Database& db, size_t nrThreads,
                                               size_t vectorSize) {
   using namespace vectorwise;
   ...
   workers.run([&]() {
      Q32Builder builder(db, shared, vectorSize);
