   ...
   #define MADV_HUGEPAGE 0
     ...
     public:
      ...
      Vector(Vector<T, AT>&& oth)
         : _size(std::exchange(oth._size, 0)), _capacity(std::exchange(oth._capacity, 0)),
      ...
      void remap(size_t size) {
         ...
         if (AT == MALLOC) {
            ...
            if (AT == TRANSHUGEPAGE) {
               ...
               // Make 'unused' warning go away
               (void) (adv);
            }
         }

         if (_capacity * sizeof(T) < space)
            memset(static_cast<char*>(mem) + _capacity * sizeof(T), 0, space - _capacity * sizeof(T));
