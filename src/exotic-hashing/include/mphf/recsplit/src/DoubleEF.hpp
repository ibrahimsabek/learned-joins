 ...
 */
   ...
   class DoubleEF {
     private:
      ...
      __inline static void set_bits(util::Vector<uint64_t, AT>& bits, const uint64_t start, const int width,
      ...
      __inline size_t jump_size_words() const {
         ...
         if (num_buckets % super_q != 0)
            size += (1 + ((num_buckets % super_q + q - 1) / q + 3) / 4) * 2; // Partial block
         return size;
      }
     ...
     public:
      ...
      DoubleEF(const std::vector<uint64_t>& cum_keys, const std::vector<uint64_t>& position) {
         ...
         u_position = int64_t(position[num_buckets]) - int64_t(bits_per_key_fixed_point * cum_keys[num_buckets] >> 20) -
            int64_t(num_buckets * min_diff) + 1;
         ...
         for (uint64_t i = 0, cum_delta = 0, bit_delta = 0; i <= num_buckets;
              i++, cum_delta += cum_keys_min_delta, bit_delta += min_diff) {
            if (l_cum_keys != 0)
               set_bits(lower_bits, i * (l_cum_keys + l_position), l_cum_keys,
                        (cum_keys[i] - cum_delta) & lower_bits_mask_cum_keys);
            ...
            if (l_position != 0)
               set_bits(lower_bits, i * (l_cum_keys + l_position) + l_cum_keys, l_position,
                        (pval - bit_delta) & lower_bits_mask_position);
         ...
         for (uint64_t i = 0, c = 0, last_super_q = 0; i < words_cum_keys; i++) {
            for (int b = 0; b < 64; b++) {
               if (upper_bits_cum_keys[i] & UINT64_C(1) << b) {
                  if ((c & super_q_mask) == 0)
                     jump[(c / super_q) * (super_q_size * 2)] = last_super_q = i * 64 + b;
                  if ((c & q_mask) == 0) {
         ...
         for (uint64_t i = 0, c = 0, last_super_q = 0; i < words_position; i++) {
            for (int b = 0; b < 64; b++) {
               if (upper_bits_position[i] & UINT64_C(1) << b) {
                  if ((c & super_q_mask) == 0)
                     jump[(c / super_q) * (super_q_size * 2) + 1] = last_super_q = i * 64 + b;
                  if ((c & q_mask) == 0) {
                     const uint64_t offset = i * 64 + b - last_super_q;
                     if (offset >= (1 << 16))
                        abort();
                     ((uint16_t*) (&jump + (c / super_q) * (super_q_size * 2) + 2))[2 * ((c % super_q) / q) + 1] =
                        offset;
                  }
                  c++;
               }
      ...
      void get(const uint64_t i, uint64_t& cum_keys, uint64_t& cum_keys_next, uint64_t& position) const {
         ...
         const uint64_t jump_cum_keys =
            jump[jump_super_q] + ((uint16_t*) (&jump + jump_super_q + 2))[2 * jump_inside_super_q];
         const uint64_t jump_position =
            jump[jump_super_q + 1] + ((uint16_t*) (&jump + jump_super_q + 2))[2 * jump_inside_super_q + 1];
         ...
         for (uint64_t bit_count; (bit_count = nu(window_cum_keys)) <= delta_cum_keys; delta_cum_keys -= bit_count)
            window_cum_keys = upper_bits_cum_keys[++curr_word_cum_keys];
         for (uint64_t bit_count; (bit_count = nu(window_position)) <= delta_position; delta_position -= bit_count)
         ...
         cum_keys =
            ((curr_word_cum_keys * 64 + select_cum_keys - i) << l_cum_keys | (lower & lower_bits_mask_cum_keys)) +
            cum_delta;

         lower >>= l_cum_keys;
         const int64_t bit_delta = i * min_diff;
         position = ((curr_word_position * 64 + select64(window_position, delta_position) - i) << l_position |
                     (lower & lower_bits_mask_position)) +
            bit_delta + int64_t(bits_per_key_fixed_point * cum_keys >> 20);
         ...
         lower >>= l_position;
         cum_keys_next = ((curr_word_cum_keys * 64 + rho(window_cum_keys) - i - 1) << l_cum_keys |
                          (lower & lower_bits_mask_cum_keys)) +
