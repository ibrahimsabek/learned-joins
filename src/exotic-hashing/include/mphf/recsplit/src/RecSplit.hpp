...
namespace sux::function {
   ...
   static const uint64_t start_seed[] = {
      0x106393c187cae21a, 0x6453cec3f7376937, 0x643e521ddbd2be98, 0x3740c6412f6572cb, 0x717d47562f1ce470,
 ...
 */
   ...
   static constexpr inline uint64_t min(int64_t x, int64_t y) {
      return y + ((x - y) & ((x - y) >> 63));
   }
   static constexpr inline uint64_t max(int64_t x, int64_t y) {
      return x - ((x - y) & ((x - y) >> 63));
   }

   // Optimal Golomb-Rice parameters for leaves.
   static constexpr uint8_t bij_memo[] = {0,  0,  0,  1,  3,  4,  5,  7,  8,  10, 11, 12, 14,
                                          15, 16, 18, 19, 21, 22, 23, 25, 26, 28, 29, 30};

#ifdef MORESTATS
   // Optimal Golomb code moduli for leaves (for stats).
   static constexpr uint64_t bij_memo_golomb[] = {
      0,        0,        1,         3,         7,          18,         45,          113,         288,        740,
 ...
 */
   ...
   class SplittingStrategy {
     ...
     public:
      ...
      split_iterator begin() {
         return split_iterator(this);
      }
      split_iterator end() {
         return split_iterator(nullptr);
      }

      inline size_t fanout() {
         return this->_fanout;
      }
   };
   ...
   static constexpr void _fill_golomb_rice(const int m, array<uint32_t, MAX_BUCKET_SIZE>* memo) {
      ...
      for (size_t i = 0; i < fanout; ++i)
         sqrt_prod *= gcem::sqrt(k[i]);

      const double p = gcem::sqrt(m) / (gcem::pow(2 * M_PI, (fanout - 1.) / 2) * sqrt_prod);
      auto golomb_rice_length = (uint32_t) gcem::ceil(
         gcem::log2(-gcem::log((gcem::sqrt(5) + 1) / 2) / gcem::log1p(-p))); // log2 Golomb modulus
   ...
   static constexpr array<uint32_t, MAX_BUCKET_SIZE> fill_golomb_rice() {
      ...
      for (; s < MAX_BUCKET_SIZE; ++s)
         _fill_golomb_rice<LEAF_SIZE>(s, &memo);
      return memo;
   }
   ...
   static constexpr array<uint8_t, MAX_LEAF_SIZE> fill_bij_midstop() {
      ...
      for (int s = 0; s < MAX_LEAF_SIZE; ++s)
         memo[s] = s < (int) gcem::ceil(2 * gcem::sqrt(s)) ? s : (int) gcem::ceil(2 * gcem::sqrt(s));
      return memo;
   }
   ...
   class RecSplit {
	 ...
	 */
      RecSplit(ifstream& input, const size_t bucket_size) {
         this->bucket_size = bucket_size;
         vector<hash128_t> h;
         for (string key; getline(input, key);)
	 ...
	 */
      size_t operator()(const hash128_t& hash) const {
         ...
         if (m > lower_aggr) {
            ...
            if (part)
               reader.skipSubtree(skip_nodes(lower_aggr) * part, skip_bits(lower_aggr) * part);
            level++;
         }
         ...
         if (m > _leaf) {
            ...
            cum_keys += _leaf * part;
            if (part)
               reader.skipSubtree(part, skip_bits(_leaf) * part);
            level++;
         }
	 ...
	 */
     ...
     private:
         ...
         if (m <= _leaf) {
            ...
            if constexpr (_leaf <= 8) {
               for (;;) {
                  ...
                  for (size_t i = start; i < end; i++)
                     mask |= uint32_t(1) << remap16(remix(bucket[i] + x), m);
#ifdef MORESTATS
                  num_bij_evals[m] += m;
#endif
                  if (mask == found)
            ...
            if (m > upper_aggr) { // fanout = 2
...
#ifdef MORESTATS
               time_split[min(MAX_LEVEL_TIME, level)] +=
                  duration_cast<nanoseconds>(high_resolution_clock::now() - start_time).count();
#endif
               recSplit(bucket, temp, start, start + split, builder, unary, level + 1);
               if (m - split > 1)
                  recSplit(bucket, temp, start + split, end, builder, unary, level + 1);
#ifdef MORESTATS
               else
                  sum_depths += level;
#endif
               ...
               for (;;) {
                  ...
                  size_t broken = 0;
                  for (size_t i = 0; i < fanout - 1; i++)
                     broken |= count[i] - lower_aggr;
                  if (!broken)
                     break;
                  x++;
               }

               for (size_t i = 0, c = 0; i < fanout; i++, c += lower_aggr)
                  count[i] = c;
               for (size_t i = start; i < end; i++) {
               ...
               if (m - i > 1)
                  recSplit(bucket, temp, start + i, end, builder, unary, level + 1);
#ifdef MORESTATS
               else
                  sum_depths += level;
#endif
               ...
               for (;;) {
                  ...
                  size_t broken = 0;
                  for (size_t i = 0; i < fanout - 1; i++)
                     broken |= count[i] - _leaf;
                  if (!broken)
                     break;
                  x++;
               }
               for (size_t i = 0, c = 0; i < fanout; i++, c += _leaf)
                  count[i] = c;
               for (size_t i = start; i < end; i++) {
      ...
      void hash_gen(hash128_t* hashes) {
         ...
         sort(hashes, hashes + keys_count,
              [this](const hash128_t& a, const hash128_t& b) { return hash128_to_bucket(a) < hash128_to_bucket(b); });
         ...
         for (size_t i = 0, last = 0; i < nbuckets; i++) {
            ...
            for (; last < keys_count && hash128_to_bucket(hashes[last]) == i; last++)
               bucket.push_back(hashes[last].second);
         ...
         _totalBitSize = ef.bitCountCumKeys() + ef.bitCountPosition() + builder.getBits();
#ifdef STATS
         ...
         printf("\n");
         printf("Bij               count              trials                 exp               evals                 "
                "exp           tot evals\n");
         for (int i = 0; i < MAX_LEAF_SIZE; i++) {
            if (num_bij_trials[i] != 0) {
               ...
               printf("%-3d%20d%20.2f%20.2f%20.2f%20.2f%20lld\n", i, bij_count[i],
                      ...
                      (double) num_bij_evals[i] / bij_count[i], (_leaf <= 8 ? i : bij_midstop[i]) * pow(i, i) / fact,
                      num_bij_evals[i]);
            }
            fact *= (i + 1);
         }
