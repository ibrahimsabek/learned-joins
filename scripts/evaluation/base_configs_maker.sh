      ...
      if [ "$INLJ_WITH_HASH_INDEX" = 1 ]; then
         if [ "$HASH_SCHEME_AND_FUNCTION_MODE" != 0 ]; then 
            ...
            # echo -n $'using namespace exotic_hashing::support;'; echo -n $'\n\n';
             ...
             echo -n $'using RadixSplineHash = learned_hashing::RadixSplineHash<RELATION_KEY_TYPE,18,10>;'; echo -n $'\n\n';
             echo -n $'using RMIHash = learned_hashing::RMIHash<RELATION_KEY_TYPE,10>;'; echo -n $'\n\n'; #1000000(fb,wiki) 20000000(fb,wiki) 80million(osm)
